...
- [ ] **Step 1:** 将消息滚动区设置为唯一可滚动区域，输入区、快捷问题和免责声明固定在底部布局中。
- [ ] **Step 2:** 统一序序头像、消息气泡、快捷问题和发送按钮尺寸，禁用态必须可辨识。
- [ ] **Step 3:** 验证空消息、快捷问题、连续发送、长消息和小屏底部安全区。
- [ ] **Step 4:** 提交 `feat: polish xuxu chat workspace`。
