<template>
  <view class="page medication-page">
    ...
    <view class="companion-banner">
      <image class="companion-art" src="/static/illustrations/medication-companion-banner.png" mode="aspectFill" />
      <view class="companion-copy"><text class="companion-title">按时照顾自己</text><text class="companion-caption">序序陪你记住每一次</text></view>
    </view>
    ...
    <view v-if="medications.length" class="timeline">
      <view v-for="item in medications" :key="item.id" class="med-row" :class="{ done: item.checked }">
        ...
        <image class="med-icon" src="/static/icons/watercolor/medication.png" mode="aspectFit" />
...
.head-add { position: relative; display: flex; align-items: center; justify-content: center; width: 116rpx; height: 64rpx; gap: 6rpx; border: 1rpx solid rgba(255,255,255,.94); border-radius: 20rpx; background: rgba(255,253,251,.84); color: #718b8a; box-shadow: 0 10rpx 22rpx rgba(126,104,94,.08), inset 0 1rpx 0 rgba(255,255,255,.96); font-size: 20rpx; }
.head-add image { width: 40rpx; height: 40rpx; flex: none; mix-blend-mode: multiply; }.head-add text { color: #718b8a; font-size: 20rpx; }
.companion-banner { position: relative; width: 100%; aspect-ratio: 3 / 2; overflow: hidden; border: 1rpx solid rgba(255,255,255,.94); border-radius: 24rpx; background: #fffaf3; box-shadow: 0 16rpx 34rpx rgba(128,108,99,.09), inset 0 1rpx 0 rgba(255,255,255,.96); }
.companion-art { position: absolute; inset: 0; width: 100%; height: 100%; object-position: center; animation: companion-breathe 7s ease-in-out infinite alternate; }
.companion-copy { position: absolute; top: 50%; left: 26rpx; z-index: 2; display: flex; width: 31%; flex-direction: column; gap: 10rpx; transform: translateY(-50%); }
.companion-title { color: #655b60; font-size: 29rpx; font-weight: 700; line-height: 1.35; }.companion-caption { color: #9a8d8e; font-size: 20rpx; line-height: 1.55; }
@keyframes companion-breathe { from { transform: scale(1); filter: saturate(.96); } to { transform: scale(1.018); filter: saturate(1.04); } }
...
.timeline { display: flex; flex-direction: column; gap: 14rpx; border: 0; border-radius: 0; background: transparent; box-shadow: none; overflow: visible; }.med-row { position: relative; min-height: 132rpx; padding: 18rpx 14rpx 18rpx 0; border: 1rpx solid rgba(255,255,255,.92); border-radius: 22rpx; background: rgba(255,253,251,.78); box-shadow: 0 12rpx 26rpx rgba(126,104,94,.07), inset 0 1rpx 0 rgba(255,255,255,.96); }.med-row:last-child { border-bottom: 1rpx solid rgba(255,255,255,.92); }.med-row.done { opacity: .72; background: rgba(235,245,242,.68); }.time-rail { width: 76rpx; color: #9b8f92; }.rail-dot { border-color: #9fc8c0; background: #fffaf6; }.rail-dot.checked { border-color: #78afa6; background: #78afa6; }
