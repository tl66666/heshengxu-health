  ...
  <view class="page"
    ...
    ><view class="cards"
      ><button :class="['choice', { selected: kind === 'weight' }]" @tap="kind = 'weight'">
        <image src="/static/illustrations/program-weight.png" mode="aspectFill" /><view
          ><text>体重管理</text><text>从规律记录与小行动开始</text></view
        ></button
      ><button :class="['choice', { selected: kind === 'sleep' }]" @tap="kind = 'sleep'">
        <image src="/static/illustrations/program-sleep.png" mode="aspectFill" /><view
          ><text>睡眠与精力</text><text>先找回更稳定的作息</text></view
        >
      </button></view
    ...
    ><button class="submit" :loading="saving" @tap="save">开始我的计划</button></view
  >
</template>
...
async function save() {
  ...
  try {
    ...
    error.value = e instanceof Error ? e.message : '暂时无法保存计划';
  } finally {
    saving.value = false;
  }
}
